...
- Control props can be used to control the internal state of a component from
  the outside. This is similar to controlled form elements (inputs) that use the
  `value` property
- Note the final version of this exercise is disabling an eslint rule about
  hooks not being called conditionally. In this case it's ok since the hooks
  that are called conditionally don't have any impact on the React component
  itself. Also note the condition is an environment check which will be static
  throughout the entire lifetime of the app
