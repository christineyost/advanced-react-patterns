...
- It's nice to supply consumers with props that they will most likely need when
  using a component. This can be done through a custom hook. The custom hook can
  return an object with the props in it (along with any thing else that
  consumers will need), then consumers can just spread this object when using
  the component (see example in this exercise)
- A better pattern is to use a prop getter like the second part of this
  exercise. That way if a user wanted to do something in addition to the normal
  behavior, the onClick, etc. could be composed
