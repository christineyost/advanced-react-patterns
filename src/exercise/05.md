...
- If a component can be used in lots of varying ways in different scenarios, one
  option is to allow consumers to pass in a reducer so they can customize the
  behavior of the component
- In doing this, it can be useful to give consumers access to the default
  reducer so they have access to it in case they want to use it
- It can also be useful to provide consumers an object containing possible
  actions for the reducer to avoid issues with consumers using the wrong action
  types
